...
- Ledger rotation is itself auditable and **cannot** silently truncate history: for the audit-critical ledgers (`action_event`, `dream_session`, `gateway_event`) `ledger.rotate` seals a segment under the SAME `LOCK_EX` the appender takes — **no row** may follow a rotation marker in a closed segment — and re-anchors the `row_hash` chain into a tamper-evident, hash-chained segment manifest with a chained HEAD pointer; `ledger.verify_chain` discovers segments via the manifest only (never a filename glob) so a removed/renumbered **middle** segment AND a removed **HEAD** segment are both detected (`ledger.py`, `tests/test_ledger_rotation.py`). advisory: the P8 `retrieval_event-*` monthly ledger keeps its fresh-chain-per-file design and is accepted best-effort search telemetry — a removed month there is not tamper-evident by design, and it is **never** the precedent for the audit-critical policy.
...
- The model-invokable write verbs are **role-invariant**: `_tools/session_memory.py` capture and `_tools/capture.py` (feedback/value/failure) accept `--role` for attribution, but verb behavior is identical under any role value — role is attribution and human-side gating only, **never** a widening of the model's tool surface; an unknown/missing role falls back to a safe `unknown` default, while the shell surfaces always pass the explicitly resolved role (`tests/test_session_memory.py`, `tests/test_capture.py`).
...
- The dream actuator command is writable **only** through the admin-only `./oracle admin autonomy set-dream` verb (`actions.py`), which updates the `dream.*` subtree (`command`/`max_minutes`/`max_inbox_items`) and **never** touches `level`/caps/kill-switch; the verb is control-plane (`policy.require_role`) and is **never** model-reachable — `autonomy.yml` is never writable from any gateway or model-invokable path, including a restore from an untrusted backup (`actions.py`, `tests/test_actions.py`).
- The dream subprocess receives a purpose-built **narrow env**: exactly the one resolved `provider.api_key_env` credential, with every other secret-suffixed var and every `gateway.*.token_env` **scrubbed** — the single sanctioned exception to the I3/M1 scrub discipline, so gateway tokens can **never** leak into an external agent harness (`actions.py`, `tests/test_actions.py`).
